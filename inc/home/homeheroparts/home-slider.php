<?php 
if( get_sub_field('slides_image_slide_transition') == 'fade' ) {
    $data_options= 'animInFromLeft:fade-in; animInFromRight:fade-in; animOutToLeft:fade-out; animOutToRight:fade-out;';
} elseif ( get_sub_field( 'slides_image_slide_transition' ) == 'left-slidein' ){
	$data_options= 'slide-in-left';
} elseif ( get_sub_field( 'slides_image_slide_transition' ) == 'slide-out-left' ){
    $data_options= 'slide-out-left';
} elseif ( get_sub_field( 'slides_image_slide_transition' ) == 'slide-in-right' ){
    $data_options= 'slide-in-right';
} elseif ( get_sub_field( 'slides_image_slide_transition' ) == 'slide-out-right' ){
    $data_options= 'slide-out-right';
} elseif ( get_sub_field('slides_image_slide_transition') == '' ) {
    $data_options= 'animInFromLeft:fade-in; animInFromRight:fade-in; animOutToLeft:fade-out; animOutToRight:fade-out;';
} ?>


<?php if( have_rows('slides_image_slider') ):?>
        ...
        <div id="slide-wrapper">
            <div class="orbit" role="region" aria-label="Slides" data-orbit data-options="<?php echo $data_options;?>">
                  <ul class="orbit-container">
                    ...
                    <?php $count = 0; while( have_rows('slides_image_slider') ): the_row(); 
